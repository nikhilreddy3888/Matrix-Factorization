function [obj,grad,lossobj,regobj] = gradUnifiedLS(v,parameter)

Y               = parameter.Y;              %rating matrix (labels) [n,m], Ytrn
lambda1         = parameter.lambda1;        %Regularization paramete, lambda_1/2 (UG-U)      
lambda2         = parameter.lambda2;        %Regularization paramete, lambda_2/2 (VG-V)  
lambda3         = parameter.lambda3;        %Regularization paramete, lambda_3/2 (U)  
lambda4         = parameter.lambda4;        %Regularization paramete, lambda_4/2 (V)  
...
%%
regobj          = (lambda3./2).*(sum(U(:).^2))+ (lambda4./2).*(sum(V(:).^2)); 

lossobj         = 0;
lossobj         = lossobj + (0.5).*sum( YMXgt0(:).^2  );
lossobj         = lossobj + (lambda1./2).*sum(UGmatMU(:).^2);
lossobj         = lossobj + (lambda2./2).*sum(VGmatMV(:).^2);
...
dV              = dV - lambda2.* VGmatMV;   %Gradient From Group Loss
